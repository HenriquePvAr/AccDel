...
export function TableSessionPage() {
  ...
  return (
    <section className="page session-page">
      ...
      {session && <CurrentOrderDrawer items={draft} open={drawerOpen} sending={sendMutation.isPending} online={online} currentUserName={user?.name} onOpen={() => setDrawerOpen(true)} onClose={() => setDrawerOpen(false)} onQuantity={(clientId, quantity) => setDraft((current) => current.map((item) => item.clientId === clientId ? { ...item, quantity } : item))} onRemove={(clientId) => setDraft((current) => current.filter((item) => item.clientId !== clientId))} onSend={() => sendMutation.mutate()} />}
